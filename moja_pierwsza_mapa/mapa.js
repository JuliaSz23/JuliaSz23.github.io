$(document).ready(function () {
  ...
  mymap.on("click", (event) => {
    ...
    $('#object_latitude').val(lat),
    $('#object_longitude').val(lng)
...
$('#data_save').click((event)=>{
  ...
  console.log($("#object_id").val());  ///przechwycic zawartosc jakiegospola
  dane.push({
    id: $("#object_id").val(),
    date: $("#object_date").val(),
    latitude: $("#object_latitude").val(),
    longitude: $("#object_longitude").val(),
    location: $("#object_location").val(),
    description: $("#object_description").val(),
    sources: [
      {
        id: 1,
        path: "https://pl.wikipedia.org/wiki/Banie_Mazurskie",
  ...
  console.log(dane);
  //$(".modalBox").hide(); //zamyka okno po kliknięciu zapisz 
  ...
  //TUTAJ ZMIANA KOSMETYCZNA (->jeżeli return jest w jednej linijce to wtedy daje się bez return za to w nawiasach okrągłych)
...
for (let item in dane) {
  ...
  //obsługa modala do eedycji
    ...
    //filtrowanie z danych, z których uzyskujemy tablice z danymi, które chcemy edytować
    wynik_filtrowania_do_edycji = dane.filter(function(item){
      return item.id == zmienna_na_THIS;
    });

    /// to jest wpisanie przefiltrowanych danych do modala w formularz
    ...
    //zapisywanie (nadpisywanie) danych do pliku 'dane'
    $('#save_edits').click(function(event){   //jeśli ktoś na Ciebie kliknie to..
      ...
      dane.forEach((item) => {
        if (item.id == zmienna_na_THIS){
          item.id = $('#object_id_edit').val();  ///łapie odpowiednie pola w modalu
      ...
      console.log(dane);  // dla kontroli, żeby sprawdzić czy wszystko działa w konsoli F12
    });
...
// najpierw obsługa slaidera
  ...
  $("#myRange").change((event) => {
    ...
    for (let item in filtered) {
      ...
      //tu się kończy pętla generująca element listy oraz listę markerów z filtrowanych danych
      layer_group?.removeFrom(mymap);
